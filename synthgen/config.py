...
class TVBBackendConfig(BaseModel):
    """TVB neural-mass simulation parameters, defaults mirror DeepSIF
    (Sun et al. 2022; Rong et al. 2025): SigmoidalJansenRit coupling, JR
    excitability A elevated on seed regions, noise targeted on SV index 4,
    dt = 0.5 ms HeunStochastic integrator. See scripts/diagnose_tvb.py
    for the physiological acceptance checks these defaults are tuned for.
    """

    model: Literal["jansen_rit", "generic_2d_oscillator", "wilson_cowan"] = "jansen_rit"
    connectivity_scheme: str = "desikan_killiany"
    global_coupling: float = Field(default=1.0, ge=0.0, description="`a` in coupling.SigmoidalJansenRit (DeepSIF uses 1.0)")
    # Noise sigma is now derived inside the backend from `noise_std` (DeepSIF
    # std parameter, typical range 1.0-1.7) using the formula
    # sigma = (a * 3.25 * (p_max - p_min) * 0.5 * std)**2 / 2 - DeepSIF.
    noise_std: float = Field(default=1.0, ge=0.0, description="DeepSIF noise std parameter. Final sigma is derived")
    integrator_dt_ms: float = Field(default=0.5, gt=0.0)
    ...
    # DeepSIF Jansen-Rit parameters
    jr_A_baseline: float = Field(default=3.25, gt=0.0, description="Baseline excitatory PSP amplitude (mV)")
    jr_A_seed: float = Field(default=3.5, gt=0.0, description="Elevated A on seed parcels (DeepSIF uses 3.5)")
    jr_mu: float = Field(default=0.083, gt=0.0, description="Mean input firing rate (DeepSIF tests 0.08-0.087)")
    jr_v0: float = Field(default=6.0)
    jr_p_min: float = Field(default=0.03, ge=0.0)
    jr_p_max: float = Field(default=0.15, ge=0.0)
...
class GenerationConfig(BaseModel):
    ...
    writer: WriterConfig
    backend: Literal["sereega", "tvb", "mix"] = "sereega"
    mix_tvb_fraction: float = Field(default=0.2, ge=0.0, le=1.0, description="Fraction of samples generated by TVB when backend='mix'")
